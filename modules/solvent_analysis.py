...
def remove_free_solvent(molecule, rAON_atomlabels): #and counterions
    ...
    for component in structure.components:
        ...
        if flag_metal == True:
            if component.is_polymeric == False:
            ...
            if charge_unit == 0:
                ...
                #neutral units
                neutral_units = [['O'],['N'],['H'], ['O', 'O'], ['N', 'N'], ['O', 'O', 'O'], ['D']]
                flag_neutral = False
    ...
    statistics_output = {'free_solvents_output' : free_solvents_output,
...
def get_oxo(molecule, file):
    ...
    def check_entry(file):
        ...
        def get_refcode(file):
            ...
            if ref_code.isupper() == True and '_' not in ref_code:
                return ref_code
            if '_' in ref_code:
                pos = ref_code.find('_')
        ...
        if 'oxo' or 'oxa' in name:
            oxo_names = ['-oxo-', '-dioxo-', '-trioxo-','-tetraoxo-', '-pentaoxo-',
            '-hexaoxo-', '-heptaoxo-','-octaoxo-', '-nonaoxo-', '-decaoxo-',
            '-undecaoxo-', '-undecaoxa-', '-dodecaoxo-', '-bis(oxo-', '-icosaoxo-',
            '-tetracosaoxo-']
        ...
        if atom.atomic_symbol == 'O':
            if len(atom.neighbours) == 1:
                ...
                if bond[0].bond_type == 'Single':
                    ...
                    if probable_metal[0].is_metal == True:
    ...
    statistics_output = {'entry_oxo' : entry_oxo,
...
def check_solvent(solvents, binding_sites_labels, molecule, unique_sites):
    ...
    statistics_output = {'solvents_for_output' : solvents_for_output,
